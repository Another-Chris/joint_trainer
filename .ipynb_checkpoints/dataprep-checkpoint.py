...
def concat_cn():
    ...
    files = glob.glob('./data/cnceleb_wav/*/*.wav')
    ...
    for i,file in enumerate(tqdm(files)):
        ...
        if len(audio) < tlen:
            ...
            fpath = file.replace('./data/cnceleb_wav/', '')
            
        if len(cat) >= tlen:
            # fname = f'./data/cnceleb/data/{curr_label}/{curr_label}-concat-{i}.flac'
            fname = f'./data/cnceleb_wav/{curr_label}/{curr_label}-concat-{i}.wav'
...
if __name__ == '__main__':
    ...
    if args.convert:
        ...
        nprocess = 6
        files_per_process = len(files) // nprocess
        ps = []
        for i in range(nprocess):
            filesegs = files[i * files_per_process : (i+1) * files_per_process]
            p = multiprocessing.Process(target = convert, args = (filesegs, ))
            p.start()
            ps.append(p)
