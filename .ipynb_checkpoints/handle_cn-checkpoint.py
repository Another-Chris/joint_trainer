    ...
    for i, file in enumerate(tqdm(files)):
        ...
        if trange[0] < len(audio) < trange[1]:
            if curr_label == prev_label and curr_genre == prev_genre:
                ...
                if len(cat) > 0: # change label/genre, but there are sth left
...
if __name__ == '__main__':
    ...
    if args.convert:
        files = glob.glob('./data/cnceleb/eval/*/*.flac')
        segs = 8
        items_per_segs = len(files) // segs + 1
        ps = []
        for s in range(segs):
    ...
    # >= tlen
    if args.build_train:
        files = glob.glob('./data/cnceleb/data/*/*.wav') # build train for wav
        target = './data/cnceleb_train.txt'        
        build_train_list(files, target, tlen = tlen)
        
        
    # save to the same root path; concat < tlen
    if args.concat:
        files = sorted(glob.glob('./data/cnceleb/data/*/*.wav'))
        concat_cn(files, (0, tlen), target_path = './data/cnceleb/data')
