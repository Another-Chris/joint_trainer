...
parser.add_argument('--save_path', 	type=str,
                    default="data", help='Target directory')
parser.add_argument('--user', 		type=str,
                    default="voxceleb1912", help='Username')
parser.add_argument('--password', 	type=str,
                    default="0s42xuw6", help='Password')
...
parser.add_argument('--convert',  dest='convert',
                    action='store_true', help='Enable convert')
...
def download_one_part(args, url, outfile, md5gt):
...
if __name__ == "__main__":
    ...
    if args.download:
        ps = []
        for line in fileparts:
            url = line.split()[0]
            md5gt = line.split()[1]
            outfile = url.split('/')[-1]
            p = multiprocessing.Process(
                target=download_one_part, args=(args, url, outfile, md5gt))
            p.start()
            ps.append(p)
        for p in ps:
            p.join()
