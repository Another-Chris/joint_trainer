    ...
    def __init__(self, inplanes, emb_dim, dilations=[1, 6, 12, 18], fmaps=48, dense=False):
        ...
        if not dense:
            ...
            self.aspp1 = _ASPPModule(
                inplanes, fmaps, 1, padding=0, dilation=dilations[0])
            self.aspp2 = _ASPPModule(
                inplanes, fmaps, 3, padding=dilations[1], dilation=dilations[1])
            self.aspp3 = _ASPPModule(
                inplanes, fmaps, 3, padding=dilations[2], dilation=dilations[2])
            self.aspp4 = _ASPPModule(
                inplanes, fmaps, 3, padding=dilations[3], dilation=dilations[3])

            self.global_avg_pool = nn.Sequential(nn.AdaptiveAvgPool1d((1)),
                                                 nn.Conv1d(
                                                     inplanes, fmaps, 1, stride=1, bias=False),
            ...
            self.aspp1 = _ASPPModule(
                inplanes, fmaps, dilations[0], padding=0, dilation=1)
            self.aspp2 = _ASPPModule(
                inplanes, fmaps, dilations[1], padding=dilations[1]//2, dilation=1)
            self.aspp3 = _ASPPModule(
                inplanes, fmaps, dilations[2], padding=dilations[2]//2, dilation=1)
            self.aspp4 = _ASPPModule(
                inplanes, fmaps, dilations[3], padding=dilations[3]//2, dilation=1)
...
class ASPP2d(Model):
    def __init__(self, inplanes, emb_dim, dilations=[1, 6, 12, 18], fmaps=48, dense=False):
        ...
        if not dense:
            ...
            self.aspp1 = _ASPPModule2d(
                inplanes, fmaps, 1, padding=0, dilation=dilations[0])
            self.aspp2 = _ASPPModule2d(
                inplanes, fmaps, 3, padding=dilations[1], dilation=dilations[1])
            self.aspp3 = _ASPPModule2d(
                inplanes, fmaps, 3, padding=dilations[2], dilation=dilations[2])
            self.aspp4 = _ASPPModule2d(
                inplanes, fmaps, 3, padding=dilations[3], dilation=dilations[3])
    ...
    def __init__(self, in_channel, out_channel, kernel_size, stride, dilations, fmaps, pool2d=False, dense=False):
        ...
        if pool2d:
            ...
            self.block1 = nn.Sequential(ASPP(in_channel, out_channel, dilations, fmaps, dense),
                                        nn.Conv1d(out_channel, out_channel, kernel_size=kernel_size,
            ...
            self.block2 = nn.Sequential(ASPP(out_channel, out_channel, dilations, fmaps, dense),
                                        nn.Conv1d(
                                            out_channel, out_channel, kernel_size=kernel_size, stride=1, padding=padding, bias=False),
                                        nn.BatchNorm1d(out_channel),
                                        nn.ReLU(out_channel))
